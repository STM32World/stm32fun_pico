...
// Global or static variable to store the Core 1 alarm pool
static alarm_pool_t *core1_alarm_pool = NULL;
...
void universal_tick_init_core1() {
    static struct repeating_timer timer;

    // Ensure the pool has been created before adding a timer to it
    if (core1_alarm_pool != NULL) {
        alarm_pool_add_repeating_timer_ms(
            core1_alarm_pool, // <--- Pass the Core 1 pool here
            -1,
            on_timer_tick,
            NULL,
            &timer);
...
void core1_entry() {
    ...
    // // Dynamically safely claim an unused alarm and build the pool in one shot
    // // Arguments: max_timers (16)
    // core1_alarm_pool = alarm_pool_create_with_unused_hardware_alarm(16);

    // // 2. Initialize your tick timer using this specific pool
    // universal_tick_init_core1();
